...
float OptimizedTDistributionScaleEstimator::compute(const cv::Mat& errors) const
{
  ...
  //while (std::abs(lambda - initial_lamda)>(1e-3)+0.01)
  //while (std::abs(lambda - initial_lamda) > (1e-3)+0.01 && std::abs(lambda - initial_lamda) < (1e-3)+0.012 );
  while (std::abs(lambda - initial_lamda) > 1e-3); // original is 3.

  /*if (std::sqrt(1.0 / lambda) > 6.5)
	  return 6.94;
  else
  return std::sqrt(1.0f / lambda);*/
  //return 6.9479;
...
float MADScaleEstimator::compute(const cv::Mat& errors) const
{
  ...
  // added on 11/11/2022.
  //cv::Mat uD = cv::Mat::ones(cv::Size(errors.cols, errors.rows), CV_32F)*cv::mean(errors).val[0];
  //abs_error = cv::abs(errors-uD);
  abs_error = cv::abs(errors); // original. 11/11/2022

  float hmin = 0, hmax = 255, hbinWidth = 1;        // Added on 26/10/2022. width = 1 is the original.

  // recycle error_hist
  //dvo::util::compute1DHistogram(error_median_absdiff, error_hist, 0, 255, 1);
  compute1DHistogram(abs_error, error_hist, hmin, hmax, hbinWidth);
  median = computeMedianFromHistogram(error_hist, hmin, hmax);

  return NORMALIZER * median;
}

const float MADScaleEstimator::NORMALIZER = 1.48f;  // The original of DVO is 1.48. ADDEDON
//const float MADScaleEstimator::NORMALIZER = 1.4799f;  // A value used in papers in general. ADDEDON
...
ScaleEstimator* ScaleEstimators::get(ScaleEstimators::enum_t type)
{
  ...
  // Addedon 24/10/2022 using approximate.
  static ApproximateTDistributionScaleEstimator ApproxTDistribution;
  ...
  switch(type)
  {
	...
	case ScaleEstimators::ApproxTDistribution:               // Addedon 24/10/2022.
		return (ScaleEstimator*)&ApproxTDistribution;
    default:
      break;
  }
  assert(false && "Unknown scale estimator type!");
...
void WeightCalculation::calculateScale(const cv::Mat& errors)
{
	...
	scale_ = std::max(scaleEstimator_->compute(errors), 0.001f);  // original
}
